...
class ShapeReco(inkex.Effect):
    ...
    def removeSmallEdge(self, paths, wTot,hTot):
        ...
        for p in paths:
            ...
            ## shouldn't this be paths[0] and paths[-1], no it shouldn't for the code flow this ius just very odd coding that definatly isn't clear.
            ...
            # 0.05 threaadhold needs to be parameterized, it's half of the 0.1 threashold below
            # remove last or first very small in anycase
            doRemove = prev==next and (diag < 0.05*(wTot+hTot)*0.5 )
            if not doRemove:
                # check if this small
                # 0.1 threaadhold needs to be parameterized
                isLarge = diag > (next.length+prev.length)*0.1  # check size relative to neighbour
                #thge 0.2 threashold needs to be paramaaterized, it's independent of the threashol above.
    ...
    def segsFromTangents(self,svgCommandsList, refNode):
        ...
        ### this should loop until no more merging cxan be done.
